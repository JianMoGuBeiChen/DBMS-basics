    ...
    if (!editing) {
      editableFields.forEach((field) => {
        row.children[field.column].innerHTML =
          `<input class="edit-input" type="${field.type}" value="${crime[field.key]}">`;
      });
      ...
      const response = await fetch(`${API_URL}/crimes/${crime["Crime Id"]}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
...
searchForm.addEventListener("submit", async (event) => {
  ...
  const url = `${API_URL}/crimes?` + params.toString();
  ...
  crimes.forEach((crime, index) => {
    crimeTable.appendChild(createRow(crime, index));
  });
});
